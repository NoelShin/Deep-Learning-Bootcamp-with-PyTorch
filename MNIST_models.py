...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        model = [nn.Linear(in_features=28 * 28, out_features=512), nn.LeakyReLU(inplace=True, negative_slope=0.2)]
        model += [nn.Linear(in_features=512, out_features=256), nn.LeakyReLU(inplace=True, negative_slope=0.2)]
        model += [nn.Linear(in_features=256, out_features=1), nn.Sigmoid()]
        self.model = nn.Sequential(*model)
