...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=[0], std=[0.5])])
    ...
    torch.save(model, './basic_classification/MNIST_model_{}.pt'.format(MODEL))
    ...
    plt.ylabel('Loss')
    plt.savefig('./basic_classification/MNIST_Loss_graph_{}.png'.format(MODEL))
    plt.show()
    ...
    plt.xlabel('Iteration')
    plt.ylabel('Accuracy (%)')
    plt.savefig('./basic_classification/MNIST_Accuracy_graph_{}.png'.format(MODEL))
    plt.show()
